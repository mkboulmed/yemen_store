...
class CheckoutBloc {
  ...
  Future updateOrderReview() async {
    final response = await apiProvider.post('/wp-admin/admin-ajax.php?action=build-app-online-update_order_review',
        Map());
    if (response.statusCode == 200) {
  ...
  Future processCredimaxPayment(String redirect) async {
    ...
    await apiProvider.processExternalPaymentToken('https://credimax.gateway.mastercard.com/api/page/version/49/pay', body);
    _placingOrderFetcher.sink.add(false);
    return true;
  }

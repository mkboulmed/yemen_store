...
class _StoreListState extends State<StoreList> {
  ...
  Widget build(BuildContext context) {
    ...
    int count = widget.stores.length;

    if(widget.stores.length > 0 && widget.block.headerAlign != 'none') {
      count = widget.stores.length + 1;
    }
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text(parseHtmlString(widget.stores[index].name), style: Theme.of(context).textTheme.headlineSmall!.copyWith(

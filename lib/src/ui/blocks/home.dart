...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  Widget buildRecentProductGridList(BlocksModel snapshot) {
    return GridView.count(
      ...
      //staggeredTiles: snapshot.recentProducts.map<StaggeredTile>((_) => StaggeredTile.fit(2)).toList(),
      mainAxisSpacing: 0.0,
      crossAxisSpacing: 0.0,
    );
  }
    ...
    switch (appBarStyle.appBarType) {
      case 'STYLE1':
        return AppBar(
          ...
          actions: [
            for (var i = 0; i < appBarStyle.actions.length; i++)
              IconButton(onPressed: () {
                onItemClick(appBarStyle.actions[i], context);
              }, icon: DunesIcon(iconString: appBarStyle.actions[i].leading)),

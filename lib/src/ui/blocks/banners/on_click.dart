...
import 'package:app/src/ui/accounts/orders/subscription_orders.dart';
import 'package:app/src/ui/blocks/app_pages/view_page.dart';
import 'package:app/src/ui/blocks/menu_groups/menu_groups.dart';
...
onItemClick(Child data, BuildContext context) async {
  if (data.linkId.isNotEmpty) {
    if (data.linkType == 'category') {
      ...
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => WebViewPage(url: data.linkId, title: data.title, elementId: data.description)));
      ...
      if(data.linkId.contains('store-manager') && AppStateModel().user.id == 0) {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    WebViewPage(url: data.linkId, title: data.title, elementId: data.description)));
      }
    } else if (data.linkType == 'webViewSAL') {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  WebViewPageSAL(url: data.linkId, title: data.title, elementId: data.description)));
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) =>
                  ProductsWidget(filter: filter, name: data.title)));
    } else if (data.linkType == 'appPage') {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => ViewPage(id: data.linkId)));
    } else if (data.linkType == 'menuGroup') {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => ViewGroups(id: data.linkId)));
      ...
      try {
        ...
        if(AppStateModel().blocks.settings.dynamicLink.isNotEmpty) {
          ...
          Share.share('check out this app ' + AppStateModel().blocks.settings.iosShareAppLink, subject: 'Check this app!');
        }
      ...
      if(AppStateModel().user.id == 0 && ['login', 'wishList', 'orders', 'wallet', 'rewardPoints', 'address', 'downloads', 'referAndEarn', 'chatRoom', 'chat', 'deleteAccount', 'bookings'].contains(data.linkType)) {
        Navigator.push(context, MaterialPageRoute(builder: (context) => Login()));
      ...
      } else if(data.linkType == 'mysubscriptions') {
        Navigator.push(context, MaterialPageRoute(builder: (context) => SubscriptionOrders()));

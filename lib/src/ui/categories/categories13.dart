...
import 'package:app/src/ui/categories/search_field.dart';
...
class _Categories13State extends State<Categories13> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          appStateModel.blocks.localeText.categories,
        ),
        //centerTitle: false,
        actions: [
...
class CategoryRow extends StatelessWidget {
  ...
  CategoryRow({required this.category});
    ...
    return Column(
      children: [
        ListTile(
          ...
          onTap: () {
            ...
            if(categories.length > 0) {
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          SubCategoriesPage(categories: categories, category: category)));
            } else {
              onCategoryClick(category, context);
            }
          },
          ...
          subtitle: category.description.isEmpty ? null : Text(category.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.bodySmall!.copyWith(
            fontSize: 12,
...
class _SubCategoriesPageState extends State<SubCategoriesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(parseHtmlString(widget.category.name))),
      body: ScopedModelDescendant<AppStateModel>(
        builder: (context, child, model) {
...
class ListTile extends StatelessWidget {
  ...
  TextStyle _titleTextStyle(ThemeData theme, ListTileThemeData tileTheme) {
    ...
    final Color? color = _textColor(theme, tileTheme, theme.textTheme.bodyLarge!.color);
    return _isDenseLayout(theme, tileTheme)
        ? theme.textTheme.bodyLarge!.copyWith(fontSize: 13.0, color: color)

import 'package:app/src/models/vendor/product_variation_model.dart';
...
final Order order;
   AddProductList({Key? key, required this.vendorBloc, required this.order}) : super(key: key);
  @override
...
class _AddProductListState extends State<AddProductList> {
  ...
  Widget buildList( AsyncSnapshot<List<VendorProduct>> snapshot) {
    return SliverList(
        delegate: SliverChildBuilderDelegate(
            ...
            return ProductItem(vendorBloc: widget.vendorBloc, product: snapshot.data![index],order: widget.order, variation: ProductVariation.fromJson({}),);
          },
  ...
  void _addProduct(VendorProduct product) {
    ...
    final lineItem = LineItem.fromJson({'id': product.id, 'quantity': 1});

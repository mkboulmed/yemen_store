  ...
  buildBody(BuildContext ctxt, ProductAttribute productAttribute) {
    ...
    return Column(
      children: [
        ...
        widget.product.attributes.any((element) => element.id == productAttribute.id) ? SwitchListTile(
            title: Text('Used for variation'),
            value: widget.product.attributes.singleWhere((element) => element.id == productAttribute.id).variation, onChanged: (value) {
          setState(() {

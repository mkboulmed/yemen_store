...
import '../../../config.dart';
import '../products_widgets/product_addons.dart';
...
import '../products_widgets/add_button_detail.dart';
...
class _ProductDetail2State extends State<ProductDetail2> {
    ...
    return StreamBuilder<Product>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            return Scaffold(appBar: AppBar(), body: Center(child: LoadingIndicator()));
          }
        });
  }
  ...
  _buildSlivers(BuildContext context, Product product) {
    ...
    list.add(buildLisOfRelatedProducts());
        ...
        child: Column(
          ...
          children: [
            ...
            if(product.sku != null && product.sku!.isNotEmpty)
              Column(
                children: [
                  Text('SKU: '+ product.sku!, style: Theme.of(context).textTheme.caption),
                ],
              ),
          ],
        ),
  ...
  Widget buildLisOfRelatedProducts() {
    String title =
  ...
  Widget buildProductList(
    ...
    if (products.length > 0) {
      return widget.appStateModel.blocks.settings.relatedProductScroll ? SliverPadding(
        padding: const EdgeInsets.fromLTRB(8.0, 16.0, 4.0, 4.0),

            ...
            child: Column(
              children: accountGroup.menuItems.map((item) {
                if (['vendorProducts', 'vendorOrders', 'vendorWebView']
                    .contains(item.linkType) &&
                    !model.isVendor.contains(model.user.role)) {
                  return Container();
                } else if (item.linkType == 'login' &&
                    model.user.id != 0) {
                  return Container();
                } else if (item.linkType == 'logout' &&
                    model.user.id == 0) {
                  return Container();
                } else {
                  return Column(
                    children: [
                      ListTile(
                        ...
                        leading: item.leading.isNotEmpty ? DunesIcon(iconString: item.leading, color: trailingIconColor) : null,
                        trailing: item.trailing.isNotEmpty ? DunesIcon(iconString: item.trailing, color: trailingIconColor) : null,
                        title: Text(item.title, style: titleStyle),
                        subtitle:
                        item.description.isNotEmpty
                            ? Text(item.description, style: subtitleTextStyle)
                            : null,
                      ),
                      item == accountGroup.menuItems.last ? Container() : Divider(thickness: 0),
                    ],
                  );
                }
              }).toList(),

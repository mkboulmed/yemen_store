...
class ShippingPayment extends StatefulWidget {
  ...
  ShippingPayment({Key? key, required this.checkoutBloc}) : super(key: key);
...
class _ShippingPaymentState extends State<ShippingPayment> {
  ...
  @override
  void initState() {
    widget.checkoutBloc.updateOrderReview();
    widget.checkoutBloc.orderResult.listen((result) => processOrder(result));
    super.initState();
  }

  void handlePaymentMethodChanged(String value) {
    widget.checkoutBloc.choosePayment(value);
    setState(() {
      _selectedPaymentMethod = value;
    });
    widget.checkoutBloc.formData['payment_method'] = value;
  }

  void handleShippingMethodChanged(String value) {
    widget.checkoutBloc.chooseShipping(value);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<OrderReviewModel>(
          stream: widget.checkoutBloc.orderReview,
          builder: (context, snapshot) {
            if (snapshot.hasData &&
                widget.checkoutBloc.orderReviewData.paymentMethods != null) {
              for (var i = 0;
                  i < widget.checkoutBloc.orderReviewData.paymentMethods.length;
                  i++) {
                if (widget.checkoutBloc.orderReviewData.paymentMethods[i].chosen ==
                    true) {
                  _selectedPaymentMethod =
                      widget.checkoutBloc.orderReviewData.paymentMethods[i].id;
                }
              }
            }
            return Padding(
                          ...
                          child: Column(
                            ...
                            children: <Widget>[
                              RaisedButton(
                                ...
                                onPressed: () {
                                  widget.checkoutBloc.placeOrder();
                                },
                                child: StreamBuilder<bool>(
                                    stream: widget.checkoutBloc.placingOrder,
                                    builder: (context, snapshot) {
                                      if (snapshot.hasData &&
                                          snapshot.data == true) {
                                        return Container(
                              ...
                              StreamBuilder<OrderResult>(
                                  stream: widget.checkoutBloc.orderResult,
                                  builder: (context, snapshot) {
                                    if (snapshot.hasData &&
  ...
  processOrder(OrderResult orderResult) async {
    if (orderResult.result == 'success') {
      if (_selectedPaymentMethod == 'cod' ||
        ...
        bool status = await widget.checkoutBloc.processCredimaxPayment(orderResult.redirect);
        openWebView(orderResult.redirect);
      } else {
        openWebView(orderResult.redirect);
      }

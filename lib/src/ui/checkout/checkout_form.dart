    ...
    snapshot.data!.fieldgroups.billing.forEach((element) {
      ...
      if(element.label.isNotEmpty) {
        if((element.type.isEmpty || element.type == 'text') && element.label != 'Delivery Location' && !['lpac_longitude', 'lpac_latitude', 'lpac_is_map_shown', 'lpac_places_autocomplete'].contains(element.key)) {
          ...
          if(_textEditingController[element.key]!.text != element.value!) {
            _textEditingController[element.key]!.text = element.value!;
          }
          ...
          list.add(
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(element.label),
                  DropdownButtonFormField<DotappOption>(
                    value: selectedOption,
                    hint: Text(element.label),
                    //disabledHint: Text(element.label),
                    isExpanded: true,
                    icon: Icon(Icons.arrow_drop_down),
                    iconSize: 24,
                    elevation: 16,
              ...
              list.add(
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(element.label, style: Theme.of(context).textTheme.titleSmall),
                      DropdownButtonFormField<String>(
                        value: element.value,
                        hint: Text(element.label),
                        isExpanded: true,
        ...
        if(element.type == 'city' && element.dotappOptions.length > 0) {
          ...
          if(element.dotappOptions.any((option) => option.value == element.value)) {
            ...
            selectedOption = element.dotappOptions.first;
            element.value = element.dotappOptions.first.value;
            widget.checkoutFormBloc.formData[element.key] = element.dotappOptions.first.value;
          }
          list.add(
            Padding(

...
class _CheckoutOnePageState extends State<CheckoutOnePage> {
  ...
  List<Widget> slivers(AsyncSnapshot<OrderReviewModel> snapshot,
    ...
    list.add(_applyWalletBalance(snapshot));
    ...
    list.add(SliverToBoxAdapter(
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            if (['cowpay_credit_card', 'stripe', 'stripe_cc', 'nuvei', 'paymongo'].contains(widget.checkoutFormBloc.formData['payment_method']))
              Container()
            else LoadingButton(
  ...
  _applyWalletBalance(AsyncSnapshot<OrderReviewModel> snapshot) {
    return SliverToBoxAdapter(
      child: snapshot.data!.balance != 0 && snapshot.data!.balance < double.parse(snapshot.data!.totalsUnformatted.total) ? Column(
        children: [
          CheckboxListTile(
              controlAffinity: ListTileControlAffinity.leading,
              title: Text('Apply Wallet Balance - ' + parseHtmlString(snapshot.data!.balanceFormatted)),
              value: walletChecked, onChanged: (value) async {

            if(value != null) {
              setState(() {
                walletChecked = value;
              });
              Map data = {'action': 'woo_wallet_partial_payment_update_session', 'checked': walletChecked ? '1' : '0'};
              await widget.checkoutFormBloc.applyWallet(data);
              widget.checkoutFormBloc.updateOrderReview2();
            }
          }),
          Divider()
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if (widget.checkoutFormBloc.formData['payment_method'] == 'cowpay_credit_card' || widget.checkoutFormBloc.formData['payment_method'] == 'nuvei' || widget.checkoutFormBloc.formData['payment_method'] == 'paymongo')
